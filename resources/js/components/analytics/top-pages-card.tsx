...
export function TopPagesCard({ data }: TopPagesCardProps) {
  ...
  return (
    <div className="h-[495px] space-y-4 border-r border-b p-8">
      <div>
        <h2 className="text-base font-semibold leading-none">Top Pages</h2>
      </div>
          ...
          <div>
            <div className="flex justify-between space-x-6 relative">
              <div className="relative w-full space-y-1.5 z-0">
                {data.map((page, index) => (
                  <div key={page.url} className="group w-full rounded-sm outline-offset-2 outline-0 focus-visible:outline-2 outline-blue-500 dark:outline-blue-500">
                    <div 
                      className={`flex items-center rounded-sm transition-all h-8 bg-green-100 hover:bg-green-200 ${index === data.length - 1 ? 'mb-0' : ''}`} 
                      style={{ width: `${(page.mentions / maxMentions) * 100}%` }}
                    >
                      <div className="absolute left-2 flex gap-2 max-w-full pr-2">
                        <p className="truncate whitespace-nowrap text-sm">{page.title}</p>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div>
                {data.map((page, index) => (
                  <div key={`${page.url}-count`} className={`flex items-center justify-end h-8 ${index === data.length - 1 ? 'mb-0' : 'mb-1.5'}`}>
                    <p className="truncate whitespace-nowrap text-sm leading-none text-muted-foreground">
                      {(page.mentions / 1000).toFixed(1)}k
                    </p>
                  </div>
                ))}
              </div>
